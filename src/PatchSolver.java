...
public class PatchSolver{
	...
	private String[] multDelStr; //String of get to the Node, that is reached by the best multiple Destruction
	private Node[] prev;
	...
	public PatchSolver(String inFile, String outFile){
		...
		multDelStr = new String[nOutput+1];
	...
	private void updateDel(int index, int cost, String str){
		if(cost < multDel[0][prevPrev[index].getOutLine()]){
			...
			multDelStr[prevPrev[index].getOutLine()] = prevPrev[index].getPatch();
		}
	}
	...
	public void printPatch(){
		if(inIndex){
			...
			System.out.println(prev[nMin].getInLine() + "  " + prev[nMin].getOutLine());
		}
		else{
			System.out.println(prev[0].getInLine() + "  " + prev[0].getOutLine());
			System.out.println(prev[0].getPatch());
		}
	}

	public void calculatePatch(){
		System.out.println("Calculating patch on Input File " + inFile + " and Output File " + outFile);
		System.out.println("Usint " + (inIndex?"inIndex":"outIndex"));
		if(inIndex){
